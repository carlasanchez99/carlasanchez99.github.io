---
title: "BACHMI GROUPS 25 individuals INFECTED (Group1: 4, Group2: 1, Group3: 2, Group4: 6, Group5: 6, Group6: 6)"
output:
  html_document:
    df_print: paged
...
# Differences hemo by Group only INFECTED

Timepoints and data:

-D-1: 22 (Group1: 4, Group3: 1, Group4: 5, Group5: 6, Group6: 6)
-D01: 0
-D07: 0
-D21/M0: 22 
-D22/M1: 2 (Group6: 2)
...
for (i in colnames(gem.cli.df[20:25])[-2]){
  ...
  print(grafic+stat_pvalue_manual(stat.test2[stat.test2$type==i,], label = "p.adj.signif", hide.ns = TRUE, tip.length = 0.01))
 } 
...
-D-1: 25 (Group1: 4, Group2: 1, Group3: 2, Group4: 6, Group5: 6, Group6: 6)
-D01: 25
-D07: 25
-D21/M0: 25 
...
print(grafic+stat_pvalue_manual(stat.test2[stat.test2$cell_population==i,], label = "p.adj.signif", hide.ns = TRUE, tip.length = 0.01))
 } 
```
...
All timepoints 25 participants (Group1: 4, Group2: 1, Group3: 2, Group4: 6, Group5: 6, Group6: 6). 
...
ig4<-ig2
p<-ig4[rep(grep("2", ig4$Group), 2),]
p$log10_conc<-runif(688)
ig4<-rbind(ig4,p)
stat.test <- ig4 %>%
...
for (i in colnames(igg[16:36])){
  ...
  grafic<-ggplot(igg, aes_string("Day",var_name_ticked)) + ylab(paste("log10(",i, ")"))+
          geom_boxplot(position=position_dodge(1), aes(fill=Group), outlier.shape = NA, alpha=0.6) +   geom_point(position=position_jitterdodge(jitter.width = 0.5), aes(color = Group))+ theme_bw()
  print(grafic+stat_pvalue_manual(stat.test2[stat.test2$fullname==i,], label = "p.adj.signif", hide.ns = TRUE))
} 
...
dfsignificance<-arrange(dfsignificance,Day,analyte, group)

cknorm4<-cknorm2
p<-cknorm4[rep(grep("2", cknorm4$Group), 2),]
p$log10_mfi<-runif(448)
cknorm4<-rbind(cknorm4,p)
stat.test <- cknorm4 %>%
...
grafic<-ggplot(cknorm, aes_string("Day", var_name_ticknormed)) + ylab(paste("log10(",i, ")"))+
    geom_boxplot(position=position_dodge(1), aes(fill=Group), outlier.shape = NA, alpha=0.6) +   geom_point(position=position_jitterdodge(jitter.width = 0.5), aes(color = Group))+ theme_bw()
print(grafic+stat_pvalue_manual(stat.test2[stat.test2$analyte==i,], label = "p.adj.signif", hide.ns = TRUE,  tip.length = 0.01))
...
igg13.2<-igg13.2[!(igg13.2$fullname=="IgG3-LSA1"& igg13.2$Day=="D01"),]  #Tots els low tenen NAja que els valos abans de 

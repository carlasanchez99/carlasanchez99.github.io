---
title: "BACHMI DIABLO genes M0 (high vs low) vs datasets M0"
output: html_document
date: "2023-10-30"
---

## 1. DIABLO between DEG M0 high vs low responders & datasets at M0
...
genesdifresp<-read.csv("C:/Users/csanchez1/OneDrive/BACHMI/ComparisonM0_high_low/geneslogFCorderM0_high_vs_low.csv", sep=",")

cel.rma<-cel.rma[,grep(paste("M0",sep="|"),colnames(cel.rma))]
...
genescameraresp<-genescameraresp[-23]
...
X=list(RNASeq=genest[-594], Cytokines=mfisd7, hemo=hemodiablo, DC=dcdiablo, ab=ig2diab)
...
## 2. Correlations DEG M0 & Cytokines M0 DIABLO circ >0.8.

```{r, include=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=23, fig.width=35}
# Uso de la función
for (i in (abcor)[16:24][-6]){
plot<-create_plot(i, circ, genest, mfisd7, gem.cli.df, "mfisd7")
  if (!is.null(plot)){
  print(plot$plot)
  }}
```
```{r, include=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=10, fig.width=20}
# Uso de la función
plot<-create_plot("IL-6", circ, genest, mfisd7, gem.cli.df, "mfisd7")
print(plot$plot)

```

## 3. Correlations DEG M0 & Antibodies M0 DIABLO circ >0.8.

```{r, include=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=23, fig.width=30}

plot<-create_plot("IgG-PTRAMP", circ, genest, ig2diab, gem.cli.df, "ig2diab")
print(plot$plot)
...
## 4. Correlations DEG M0 & DC M0 DIABLO circ >0.8.

```{r, include=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=14}

for (i in (abcor)[7:8]){
plot<-create_plot(i, circ, genest, dcdiablo, gem.cli.df, "dcdiablo")
  if (!is.null(plot)){
  print(plot$plot)
  }}

```
```{r, include=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=20, fig.width=26}

plot<-create_plot("Lymphos", circ, genest, dcdiablo, gem.cli.df, "dcdiablo")
print(plot$plot)
```
## 5. Correlations DEG M0 & Hemo M0 DIABLO circ >0.8.

```{r, include=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=28, fig.width=37}

for (i in (abcor)[10:15]){
...
```{r, include=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=8, fig.width=18}
...
mod <- model.matrix(~ 0+responder + sex, colData(scores))
...
data.frame(Expression = as.vector(assay(scores["GO:0098792", ])),
    responder = scores$responder) %>%
    ggplot(aes(x = responder, y = Expression, col = responder)) +
        geom_boxplot() + scale_color_manual(values = c("darkgreen","#B2DF8A"))+
        theme_bw() +
    ylab("NetActivity scores")

weights <- rowData(scores)["GO:0060339", ]$Weights_SYMBOL[[1]]
data.frame(weight = weights, gene = names(weights)) %>%
    ...
    theme_bw() +  scale_fill_manual(values = c("#B2DF8A", "darkgreen"))+
    ylab("Weight") +
    xlab("Gene")+ggtitle(topTab["GO:0060339", ]$GeneSetName)

data.frame(Expression = as.vector(assay(scores["GO:0039530", ])),
    responder = scores$responder) %>%
    ggplot(aes(x = responder, y = Expression, col = responder)) +
        geom_boxplot() + scale_color_manual(values = c("darkgreen","#B2DF8A"))+
        theme_bw() +
...
weights <- rowData(scores)["GO:0039530", ]$Weights_SYMBOL[[1]]
data.frame(weight = weights, gene = names(weights)) %>%
    mutate(Direction = ifelse(weight > 0, "Positive", "Negative")) %>%
    ggplot(aes(x = gene, y = abs(weight), fill = Direction)) + 
    geom_bar(stat = "identity") +
    theme_bw() + scale_fill_manual(values = c("#B2DF8A", "darkgreen"))+
    ylab("Weight") +
    xlab("Gene")+ggtitle(topTab["GO:0039530", ]$GeneSetName)
```
